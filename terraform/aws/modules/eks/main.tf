...
resource "aws_cloudwatch_log_group" "new-cwlog" {
...
resource "aws_eks_cluster" "new-cluster" {
  ...
  depends_on = [
    aws_cloudwatch_log_group.new-cwlog,
...
resource "aws_eks_node_group" "new-node-group-1" {
  cluster_name = aws_eks_cluster.new-cluster.name
  node_group_name = "new-node-group-1"
