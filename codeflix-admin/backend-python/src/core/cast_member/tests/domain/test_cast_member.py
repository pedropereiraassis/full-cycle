...
class TestCastMember:
    ...
    def test_name_must_have_less_than_255_characters(self):
        with pytest.raises(
            ValueError,
            match="name cannot be longer than 255",
        ):
    ...
    def test_create_cast_member_with_provided_values(self):
        cast_id = uuid.uuid4()
        cast_member = CastMember(id=cast_id, name="John Doe", type=CastMemberType.ACTOR)
...
class TestUpdateCastMember:
    def test_update_cast_member_with_name_and_type(self):
        ...
        cast_member.update_cast_member(
            name="Jane Unknown", type=CastMemberType.DIRECTOR
        )

        assert cast_member.name == "Jane Unknown"
