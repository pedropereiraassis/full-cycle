...
class VideoConvertedRabbitMQConsumer(AbstractConsumer):
    ...
    def on_message(self, message: bytes):
        ...
        try:
            ...
            processed_audio_video_media_input = ProcessAudioVideoMedia.Input(
                video_id=aggregate_id,
                encoded_location=encoded_location,
                media_type=media_type,
                status=status,
            )
            print("Calling use case with input", processed_audio_video_media_input)
            use_case = ProcessAudioVideoMedia(
                video_repository=DjangoORMVideoRepository()
            )
            use_case.execute(input=processed_audio_video_media_input)
